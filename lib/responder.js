 ...
 * @param {Object} conf
 */
function Responder(mtime, conf, opt, req, res) {
  this._conf = conf;
  this._request = req;
  this._response = res;
  this._mtime = mtime;
...
Responder.prototype._processTypeScript = function(parsed, cb) {
  try {
    ...
    var ts = pathutil.resolve(rootDir + '/' + parsed.query.path.replace(/\.js$/, '.ts'));
    ...
    var compilationResult = this._compiler.compile(resolved.resolved);
...
module.exports = function(conf) {
  var mtime = new Mtime();
  if (conf.updateAll) {
    glob.sync(conf.basePath + '/**/*.ts').forEach(function(file) {
  ...
  return function(req, res, next) {
    new Responder(mtime, conf, optionParser.getCompilationSettings(), req, res).respond(next);
  };
};
