...
function Middleware(mtime, conf, opt, req, res) {
  ...
  this._systemjsPath = this._conf.systemjsPath? '/' + pathutil.relative(this._urlRoot, pathutil.resolve(this._conf.systemjsPath)): '';
  this._systemjsConfigPath = this._conf.systemjsConfigPath? '/' + pathutil.relative(this._urlRoot, pathutil.resolve(this._conf.systemjsConfigPath)): '';
...
Middleware.prototype.respond = function(next) {
  ...
  if (this._conf.usePathname && !/(?:\.js)$/.test(parsed.pathname)) {
    return next();
  }
  var d = domain.create();
  ...
  d.run(function() {
    this._processTypeScript(parsed, next, function(success, path, opt_diagnostic) {      
      if (this._conf.respond) {
        ...
        if (success) {
          if (parsed.query.notRespond) {
            return this._response.end(fs.readFileSync(path, 'utf8'));
          }
          return this._response.end(ejs.render(DEPS_TEMPLATE, {
            ...
            systemsjs: this._systemjsPath,
            systemjsConfig: this._requirejsConfigPath
          }));
        }
